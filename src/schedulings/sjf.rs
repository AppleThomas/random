...
impl SJF {
    ...
    fn find_shortest_job(&mut self) {
        ...
        for (name, time_left) in self.job_list.iter() {
            if shortest.1 > time_left {
...
impl Scheduler for SJF {
    ...
    fn on_arrive(&mut self, process: &mut Process, time: i32) {
        self.job_list.insert(process.name.to_owned(), process.time_remaining);

        if let Some(shortest_job_name) = &self.shortest_job {
            if self.job_list[shortest_job_name] <= process.time_remaining {
                return;
            }
        }
